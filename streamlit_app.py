...
st.title("Signal Spiral & Harmonic Frequency Report")
...
def generate_spiral(harmonic, max_points=5000):
    golden_angle = np.deg2rad(137.5)
    r = np.sqrt(np.arange(1, max_points + 1))
    theta = np.arange(1, max_points + 1) * golden_angle
    x = r * np.cos(theta)
    y = r * np.sin(theta)
    return x, y, harmonic
