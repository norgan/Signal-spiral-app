...
function profile(name, dob, colour) {
  ...
  const n = year * month;
  return {
    name: (name || 'Person').trim() || 'Person', dob, year, month, day, n,
    dayMod: day, dayArm: n % day,
    monthMod: month, monthArm: n % month,
    colour
  };
}
...
function big5(person) {
  ...
  const n = person.n, m = person.dayMod, a = person.dayArm;
  return {
...
function hideTables() {
  stopTone();
  ['harmonicsSection', 'big5Section', 'relationshipSection'].forEach((id) => $(id).classList.add('hidden'));
}

function toneButtonHtml(frequency) {
  const value = Number(frequency);
  return `<button type="button" class="tone-button" data-frequency="${value.toFixed(6)}" aria-label="Play ${value.toFixed(3)} hertz">▶ ${value.toFixed(3)} Hz</button>`;
}

function renderHarmonics(people) {
  let html = '<tr><th>Person</th><th>Day fundamental</th><th>Month fundamental</th><th>Day cycles</th><th>Legacy index</th><th>Audio-mapped octave equivalents</th><th>Closest returns</th></tr>';
  for (const person of people) {
    ...
    const tones = octaves.length ? octaves.map(toneButtonHtml).join(' ') : '—';
    html += `<tr><td>${esc(person.name)}</td><td>${deg(h.dayDelta).toFixed(3)}°</td><td>${deg(h.monthDelta).toFixed(3)}°</td><td>${h.dayCycles.toFixed(6)}</td><td>${legacyIndex(person).toFixed(3)}</td><td class="tone-list">${tones}</td><td>${returns}</td></tr>`;
  }
...
async function playTone(frequency, button) {
  if (!Number.isFinite(frequency) || frequency < 20 || frequency > 20000) return;
  if (activeToneButton === button && activeOscillator) { stopTone(); return; }
  stopTone();
  const AudioCtx = window.AudioContext || window.webkitAudioContext;
  if (!AudioCtx) { alert('Web Audio is not supported in this browser.'); return; }
...
function stopTone() {
  if (activeToneButton) {
    activeToneButton.textContent = activeToneButton.dataset.originalLabel || activeToneButton.textContent.replace('■', '▶');
  ...
  if (activeOscillator && activeGain && audioContext) {
    ...
    try {
      ...
      activeGain.gain.linearRampToValueAtTime(0, now + 0.04);
...
function updateToneVolume() {
  if (!activeGain || !audioContext) return;
  const volume = Math.max(0, Math.min(0.08, Number($('toneVolume')?.value || 0.02)));
  activeGain.gain.setTargetAtTime(volume, audioContext.currentTime, 0.02);
}
...
function collectPeople() {
  return [...document.querySelectorAll('.person')].map((card, i) => profile(card.querySelector('.name').value, card.querySelector('.dob').value, COLORS[i]));
}
...
function download() {
  ...
  link.download = `signal-spiral-${mode}.png`;
...
$('toneVolume').addEventListener('input', updateToneVolume);
$('harmonicsTable').addEventListener('click', (event) => {
  const button = event.target.closest('.tone-button');
  if (!button) return;
  playTone(Number(button.dataset.frequency), button);
});
...
if (query.get('people')) {
  try {
    ...
    renderPeople();
    [...document.querySelectorAll('.person')].forEach((card, i) => {
      card.querySelector('.name').value = list[i]?.name || `Person ${i + 1}`;
      card.querySelector('.dob').value = list[i]?.dob || '';
    });
  } catch (_) {}
}
